...
async function migrateFromOldKanban(app) {
  ...
  let migrated = content.replace(/kanban-plugin:\s*board/, `kanban-plugin: ${FRONTMATTER_KEY}`);
...
class VukiKanbanSettingTab extends import_obsidian2.PluginSettingTab {
  plugin;
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Note path prefix").setDesc("Base folder path for notes created via 'Create linked note' (e.g. Projects)").addText((text) => text.setPlaceholder("Projects").setValue(this.plugin.settings.notePathPrefix).onChange(async (value) => {
...
async function selfUpdate(app) {
  ...
  const manifestResponse = await import_obsidian3.requestUrl({
  ...
  const downloads = await Promise.all(files.map(async (fileName) => {
    const response = await import_obsidian3.requestUrl({ url: `${downloadBase}/${fileName}` });
...
function createCardElement(card, columnIndex, cardIndex, board, onMutation, vault, pluginSettings, projectPill) {
  ...
  import_obsidian3.setIcon(dragHandle, "grip-vertical");
...
function showPriorityMenu(event, card, columnIndex, cardIndex, board, onMutation) {
  ...
  menu.addItem((item) => item.setIcon("circle").setTitle("None").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { priority: null });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addItem((item) => item.setIcon("alert-circle").setTitle("Important").onClick(() => {
...
function showCardContextMenu(event, card, columnIndex, cardIndex, board, onMutation, vault, pluginSettings) {
  ...
  const todayString = toDateString(new Date);
  if (card.date === todayString) {
    menu.addItem((item) => item.setIcon("sun-dim").setTitle("Remove from today").onClick(() => {
      const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { date: null });
      onMutation({ ...board, columns: newColumns });
    }));
  } else {
    menu.addItem((item) => item.setIcon("sun").setTitle("Add to today").onClick(() => {
  ...
  menu.addSeparator();
  menu.addItem((item) => item.setIcon("circle").setTitle("Priority: None").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { priority: null });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addItem((item) => item.setIcon("alert-circle").setTitle("Priority: Important").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { priority: "important" });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addSeparator();
  ...
  menu.addItem((item) => item.setIcon("calendar").setTitle("Date: Today").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { date: toDateString(today) });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addItem((item) => item.setIcon("calendar-plus").setTitle("Date: Tomorrow").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { date: toDateString(tomorrow) });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addItem((item) => item.setIcon("calendar-range").setTitle("Date: Next Monday").onClick(() => {
    const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { date: toDateString(getNextMonday()) });
    onMutation({ ...board, columns: newColumns });
  }));
  menu.addItem((item) => item.setIcon("calendar-search").setTitle("Date: Pick...").onClick(() => {
    showDatePicker(card, columnIndex, cardIndex, board, onMutation);
  }));
  if (card.date) {
    menu.addItem((item) => item.setIcon("calendar-x").setTitle("Date: Remove").onClick(() => {
      const newColumns = immutableUpdateCard(board.columns, columnIndex, cardIndex, { date: null });
      onMutation({ ...board, columns: newColumns });
    }));
  }
  menu.addSeparator();
  if (!card.linkedNote) {
    menu.addItem((item) => item.setIcon("file-plus").setTitle("Create linked note").onClick(async () => {
...
`);
    ...
    menu.addItem((item) => item.setIcon("file-x").setTitle("Delete linked note").setWarning(true).onClick(async () => {
  ...
  menu.addSeparator();
  menu.addItem((item) => item.setIcon("trash-2").setTitle("Delete card").setWarning(true).onClick(() => {
...
function createColumnElement(column, columnIndex, board, viewState, onMutation, vault, pluginSettings) {
  ...
  header.addEventListener("contextmenu", (event) => {
    event.preventDefault();
    const menu = new import_obsidian3.Menu;
    menu.addItem((item) => item.setIcon("eye-off").setTitle("Hide column").onClick(() => {
    ...
    menu.addItem((item) => item.setIcon("palette").setTitle("Color").onClick((event2) => {
      ...
      colorMenu.addItem((colorItem) => colorItem.setIcon("rotate-ccw").setTitle("Reset color").onClick(() => {
    ...
    menu.addSeparator();
    menu.addItem((item) => item.setIcon("trash-2").setTitle("Delete column").setWarning(true).onClick(() => {
  ...
  if (!isCollapsed) {
    ...
    const sortedCardIndices = column.cards.map((_, index) => index).sort((indexA, indexB) => {
...
function setButtonContent(button, iconName, label) {
  ...
  import_obsidian3.setIcon(iconSpan, iconName);
...
function openQuickAddDialog(board, onMutation, prefillDate, editContext) {
  ...
  import_obsidian3.setIcon(priorityIcon, "alert-circle");
...
function renderTodayView(container, board, viewState, onMutation, vault, pluginSettings) {
  ...
  for (const group of dateGroups) {
    ...
    for (const todayCard of group.cards) {
      ...
      const cardElement = createCardElement(todayCard.card, todayCard.columnIndex, todayCard.cardIndex, board, onMutation, vault, pluginSettings, pill);
  ...
  for (const { dateKey, element: cardListElement } of sectionCardLists) {
    const sectionSortable = import_sortablejs.default.create(cardListElement, createCardSortableOptions((event) => {
...
function renderBoardColumns(container, board, viewState, onMutation, vault, pluginSettings) {
  ...
  const sortableInstances = [];
  const columnSortable = import_sortablejs.default.create(boardElement, {
    animation: 150,
...
// src/view.ts
class KanbanView extends import_obsidian4.TextFileView {
  board = { columns: [], archivedCards: [], settings: { collapsedColumns: [], todayOrder: {}, columnColors: {} } };
  viewState = { todayFilterActive: true, hideCompletedActive: true };
  sortableInstances = [];
  plugin;
  boardContainer;
  ...
  render() {
    this.destroySortable();
    this.sortableInstances = renderBoard(this.boardContainer, this.board, this.viewState, (newBoard) => {
      this.board = newBoard;
      this.requestSave();
      this.render();
    }, (newViewState) => {
      ...
      this.render();
    }, this.app.vault, this.plugin.settings, this.app);
  }
}

// src/main.ts
class VukiKanbanPlugin extends import_obsidian5.Plugin {
      ...
      setViewState(original) {
        return function(state, ...rest) {
          if (state.type === "markdown" && state.state?.file) {
            const fileCache = self2.app.metadataCache.getCache(state.state.file);
