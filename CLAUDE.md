...
**Test infrastructure:** `src/test-utils.ts` has factories (`makeCard`, `makeColumns`, `makeBoard`). `src/test-mock-obsidian.ts` mocks the `obsidian` module via `bun:test`.
...
- **Immutable board mutations** — `immutableSpliceCard()` and `immutableUpdateCard()` in board-utils.ts. Never mutate Board directly.
