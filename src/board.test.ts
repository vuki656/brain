...
import { makeCard, makeBoard, makeTodayCard } from "./test-utils";
...
describe("isCardVisibleInTodayFilter", () => {
    it("should return false for completed card with date", () => {
        expect(isCardVisibleInTodayFilter(makeCard({ completed: true, date: "2026-03-01" }))).toBe(false);
    });
...
describe("sortCardsByOrder", () => {
    ...
    it("should place cards not in savedOrder after ordered ones", () => {
        ...
        expect(result[0].card.id).toBe("ccc");
        expect(result.slice(1).map((todayCard) => todayCard.card.id)).toContain("aaa");
        expect(result.slice(1).map((todayCard) => todayCard.card.id)).toContain("bbb");
    });
...
describe("collectCardsByDateGroup", () => {
    ...
    it("should always include 'Today' group even when empty", () => {
        ...
        const todayGroup = groups.find((group) => group.dateKey === "today");
    ...
    it("should group today's cards into 'Today'", () => {
        ...
        const board = makeBoard({
            columns: [{
                title: "Col",
                cards: [makeCard({ id: "t1", date: "2026-02-22" })],
            }],
        });
    ...
    it("should group overdue cards into 'Overdue' before 'Today'", () => {
        ...
        const board = makeBoard({
            columns: [{
                title: "Col",
                cards: [
                    makeCard({ id: "od1", date: "2026-02-20" }),
                    makeCard({ id: "t1", date: "2026-02-22" }),
                ],
            }],
        });
    ...
    it("should not include 'Overdue' group when no overdue cards", () => {
        ...
        expect(groups.find((group) => group.dateKey === "overdue")).toBeUndefined();
    });
    ...
    it("should group future cards by date sorted chronologically", () => {
        ...
        const board = makeBoard({
            columns: [{
                title: "Col",
                cards: [
                    makeCard({ id: "f2", date: "2026-02-25" }),
                    makeCard({ id: "f1", date: "2026-02-24" }),
                ],
            }],
        });

        const groups = collectCardsByDateGroup(board);
        const futureGroups = groups.filter((group) => group.dateKey !== "today" && group.dateKey !== "overdue");
    ...
    it("should apply savedOrder sorting within groups", () => {
        ...
        const board = makeBoard({
            columns: [{
                ...
                cards: [
                    makeCard({ id: "t1", date: "2026-02-22" }),
                    makeCard({ id: "t2", date: "2026-02-22" }),
                ],
            }],
            settings: {
                collapsedColumns: [],
    ...
    it("should exclude completed cards", () => {
        ...
        const board = makeBoard({
            columns: [{
                title: "Col",
                cards: [
                    makeCard({ id: "done", date: "2026-02-22", completed: true }),
                    makeCard({ id: "open", date: "2026-02-22" }),
                ],
            }],
        });
    ...
    it("should exclude cards without dates", () => {
        ...
        const board = makeBoard({
            columns: [{
                ...
                cards: [
                    makeCard({ id: "nodate" }),
                    makeCard({ id: "hasdate", date: "2026-02-22" }),
                ],
            }],
        });
    ...
    it("should preserve columnIndex, cardIndex, and columnTitle in TodayCard", () => {
        ...
        const board = makeBoard({
            columns: [
                { title: "First", cards: [makeCard({ id: "skip" })] },
                { title: "Second", cards: [makeCard({ id: "target", date: "2026-02-22" })] },
            ],
        });
    ...
    it("should handle multiple columns with mixed dates", () => {
            ...
            columns: [
                {
                    title: "Col A",
                    cards: [
                        makeCard({ id: "a1", date: "2026-02-22" }),
                        makeCard({ id: "a2", date: "2026-02-20" }),
                    ],
                },
                {
                    title: "Col B",
                    cards: [
                        makeCard({ id: "b1", date: "2026-02-22" }),
                        makeCard({ id: "b2", date: "2026-02-25" }),
                    ],
                },
            ],
        });
        ...
        const overdueGroup = groups.find((group) => group.dateKey === "overdue");
        const todayGroup = groups.find((group) => group.dateKey === "today");
        const futureGroup = groups.find((group) => group.dateKey === "2026-02-25");
